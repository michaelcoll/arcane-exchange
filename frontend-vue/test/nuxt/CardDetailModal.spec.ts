...
const baseSetList: SetInfo[] = [{ code: 'neo', name: 'Kamigawa: Neon Dynasty' }];
...
describe('CardDetailModal', () => {
  ...
  it('emits "close" when clicking the backdrop', async () => {
    wrapper = mount(CardDetailModal, {
      props: { card: baseCard, setList: baseSetList },
      global: { stubs },
    });
  ...
  it('displays the full set name when the set code is present in the set list', () => {
    wrapper = mount(CardDetailModal, {
      props: { card: baseCard, setList: baseSetList },
      global: { stubs },
    });
  ...
  it('falls back to the uppercased set code when the set code is absent from the set list', () => {
    wrapper = mount(CardDetailModal, {
      props: { card: baseCard, setList: [] },
      global: { stubs },
    });
    expect(wrapper.text()).toContain('NEO');
  });

  it('colors the set icon neutral when the set code is absent from the set list, regardless of rarity', () => {
    wrapper = mount(CardDetailModal, {
      props: { card: baseCard, setList: [] },
      global: { stubs },
    });
  ...
  it.each(Object.entries(RARITY_COLOR_CLASS) as [RarityCode, string][])(
    ...
    (rarityCode, colorClass) => {
      wrapper = mount(CardDetailModal, {
        props: { card: { ...baseCard, rarity_code: rarityCode }, setList: baseSetList },
        global: { stubs },
      });
  ...
  it('colors the set icon neutral when the rarity code is unknown', () => {
    wrapper = mount(CardDetailModal, {
      props: { card: { ...baseCard, rarity_code: 'X' as RarityCode }, setList: baseSetList },
      global: { stubs },
    });
    expect(wrapper.find('i.ss').classes()).toContain(NEUTRAL_COLOR_CLASS);
  });
